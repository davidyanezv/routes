...
- (void)viewDidLoad
{
    ...
    [[SessionManager manager] departureDetails:self.item[@"id"] completionHandler:^(id responseObject, NSError *error) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (item[@"calendar"] && [item[@"calendar"] isKindOfClass:[NSString class]] && item[@"time"] && [item[@"time"] isKindOfClass:[NSString class]]) {
        cell.textLabel.text = [NSString stringWithFormat:@"%@ %@", item[@"calendar"], item[@"time"]];
    }
    return cell;
}
